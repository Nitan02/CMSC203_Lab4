...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   ...
   {
	   int minimum = intArray[0][column]; // Initialize minimum with the first element of the column
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            ...
	            if (intArray[row][column] == value) {
	                count++; // Increment the counter if the value is found
	            }
	            animate(row, column, count);
	        }
	    }
	    
	    return count; // Return the count of occurrences of the value
   }
